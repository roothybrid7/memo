...
Machine tags and deployment mappings:
    ...
    tag :dbm, :name_prefix => "db", :tags => ["xx_db:env=prod", "xx_db:role=master"], :deployment => 12345
    # => role :dbm, ["dbXXX", ... in deployment ID:12345]
...
This can be disabled with ENV variable:`RS_CACHE=false` OR `set :rs_lifetime 0` in the Capfile.
